...
- Reading a credential means reading its PRESENCE, never its value: report `SENTRY_ACCESS_TOKEN=set (71 chars)` or `absent`, and check a generated artifact by grepping for the prefix and reporting the count. Never echo the value, never pass a pasted secret to a tool, and never ask for one through the chat - a value that must be set goes into the file by the user's own hand, or by a copy-ready command they run in their terminal. The sanctioned one-key read is `guard-secret-value.js --presence <file> [KEY ...]`, which answers `KEY=set (N chars)` or `KEY=absent`. That guard enforces the rest and needs no instruction from you: it blocks the Read tool on a credential-holding file (by content, whatever the path) and a credential-shaped literal in a command, and it REWRITES a shell dump, echo or bare `env` into its redacted view instead of blocking, so the placeholder is what the transcript holds and no turn is retried. When the VALUE itself is what the user needs - shown to them, or placed where a blind copy (`jq ... > file`, `cp`, `sed -i`) cannot reach - the block or the redacted view ends in ONE AskUserQuestion ('Presence only' recommended), and a 'show or use it' answer is honoured through the `<docs-path>/flow/SECRET-READ-ALLOW` receipt (a file, a variable name, or `*`; this session only, under 8h): the user decides, the model never does, and a remote user is not left waiting on a terminal they cannot reach.
