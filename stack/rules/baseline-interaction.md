...
- Simple but clear, never complicated: lead with the point, and put the gist of the result in bullet points when there is more than one. Depth overrides only on an explicit ask. Short comes from dropping detail, never from compressing into jargon or shorthand - a term of art the user did not introduce gets spelled out in plain words at first use, including in a final summary written long after the term was defined (measured: four audited sessions drew 'explain shorter and simpler' corrections, and one unexplained 'AC4' in a close-out forced a re-ask seven hours later). And 'I haven't understood' means re-explain PLAINER at the same short length - not longer: the measured failure swung 992 -> 3560 -> 675 chars in nine minutes because the rewrite overcorrected into a wall of text instead of simpler words; a second failed re-explain routes to the format ask below, not a third guess.
...
- A re-ask on the SAME deliverable's shape or length means the guess failed - nail the format once via AskUserQuestion instead of guessing again (measured: four re-asks regenerated one handoff doc at full session context, ~20k output). State the chosen shape inline on the first copy-paste artifact so one redirect suffices.
