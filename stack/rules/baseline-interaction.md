...
- **ANSWER BUDGET - at most 3 sentences plus bullet points, about 900 characters of prose; `guard-answer-length.js` injects it every turn and BLOCKS past 1,800.** Only the user's own message lifts it, in any language ('in detail', 'walk me through', 'write a plan', детально); 'explain' does NOT. Two things it never trims, because trimming them DESTROYS the answer: a field a skill's own report contract requires, and a self-correction disclosure.
