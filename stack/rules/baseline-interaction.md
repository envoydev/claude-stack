...
- When the answer to a status question names a change THIS session can make, offer to make it in the same turn rather than instructing the user how to (measured: an answer read 'set it to `1` to enable', the user typed 'enable it' 13.9 s later, and the assistant then made the edit itself - 85,934 re-sent tokens for the round trip). And a fact about the stack's own behaviour is READ, never guessed: the artifact that reads a setting - the rule or hook that names it - is what states what it does and when it takes effect.
