...
- **ANSWER BUDGET - at most 3 sentences plus bullet points, about 900 characters of prose; `guard-answer-length.js` injects this budget into every turn and BLOCKS the answer past 1,800.** Only the user's own message lifts it, in any language ('in detail', 'walk me through', 'write a plan', детально); 'explain' does NOT. Two things the cap never touches, because trimming them DESTROYS the answer: a field a skill's own report contract requires, and a self-correction disclosure - one forced re-answer went 3,184 to 1,085 characters and took two of five headline findings and the disclosure of a mistake with it, so the short version was, by construction, the one that no longer mentioned the error.
...
- A blocking ask - a pick, an approval, an input the work cannot proceed without - goes through the AskUserQuestion tool: concrete options, the recommended one marked (a prose-only question gets skipped in live runs, and `guard-stop-contract.js` blocks a turn that ends on one). The question TEXT carries the recommendation - the action and the one reason it matters ('Re-run the capabilities capture? 12 skill files changed, so its stamped sentences are stale') - never a contentless opener ('What now?', 'How do you want to proceed?') over a list: the options are the choices, the question is the recommendation, and an ask that carries neither hands back the deciding the report just did. Free-form prose only when no options can be named. Before marking an option Recommended, check it against the conventions the user stated in THIS conversation and against any request the run has not actioned yet; a contradiction routes to a plain, non-defaulted question instead (the same guard injects that check as the ask is built). A dispatched seat has no user channel - it returns the open question in its report instead of asking.
