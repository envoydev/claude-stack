...
- **ANSWER BUDGET - at most 3 sentences plus bullet points, about 900 characters of prose.** Lead with the result, then stop. This is the whole answer, work output and explanation alike; if the user wants more they will ask, so never pre-empt it. Code, tables and command output are exempt and do not count. Only the user's own message lifts the cap - 'in detail', 'walk me through', 'write a plan', in any language (детально, покроково); 'explain' does NOT, an explanation is capped like anything else. Mechanized, not advisory: `guard-answer-length.js` injects this budget into every turn and blocks a wall-of-text answer when the turn ends.
