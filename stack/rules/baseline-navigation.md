...
- An EMPTY reference result for a symbol that plausibly has callers is suspect, not proof: in a multi-tsconfig monorepo without composite project references, serena and the LSP share the same cross-lib blind spot - cross-check with grep/`search_for_pattern` before concluding 'no callers'.
