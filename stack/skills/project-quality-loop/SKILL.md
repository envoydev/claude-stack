...
description: "Autonomous review-and-fix pipeline driven from a folder of numbered prompt files (the `<docs-path>/loops/` folder of code-quality / naming / comments). Runs each prompt in numeric order, looping it on a target until its bar is met, then advances - making and logging judgment calls itself, never pausing past the run-start mode ask. Triggers on 'run the project quality loop' or 'run the loops pipeline'. Code-quality only - architecture restructuring is project-architecture-quality-loop; measuring or raising test coverage is the project-test-coverage-analyzer/-loop pair; a single diff sweep is /code-review or /security-review, not this. Do NOT fire for a one-off review pass or when findings should be reported without auto-fixing (a missing loops folder is no blocker - the skill creates it from a bundled starter set to bootstrap)."
disable-model-invocation: true
---

# Project Quality Loop - Review-and-Fix Folder Pipeline (Autonomous)

You drive a pipeline of review-fix loops from a folder. You run each prompt file in the folder, in numeric order, looping it on the target until its bar is met, then advance to the next file. After the run-start mode ask, run fully autonomously - never ask for input, pause for approval, or wait for a human. When a decision is needed, make it yourself, apply it, and log it.
...
- LOOP_DIR - the folder of prompt files. When not given, check `<docs-path>/loops/` - that existing folder IS the loop set; missing means BOOTSTRAP below. Each file is named `{number}.{name}.md` (e.g. `1.code-quality.md`, `2.naming.md`, `3.comments.md`) - three stages. There is no architecture stage (code-quality reads the architecture map and audits conformance to it; architecture-level restructuring is the separate `project-architecture-quality-loop` skill) and no tests stage (the GREEN BASELINE gate keeps the suite green; coverage work is the `project-test-coverage-analyzer`/`-loop` pair).
...
## EXECUTION MODES
DELEGATED vs INLINE keys on dispatch capability, not file presence - agent files on disk with no Agent tool to dispatch them is still INLINE. When dispatch is available, ask ONE question before DISCOVERY - run the pipeline in the current session, or dispatch the audit and fix seats? - then hold the answer for the run; no dispatch capability (or a TARGET with nothing to hand off, below) is INLINE without asking:
...
This skill ships a starter set under its own `references/` folder - three stage prompts: code quality, naming, and comments, all audits. The code-quality stage reads `<docs-path>/architecture/ARCHITECTURE.md` and audits TARGET for both quality and conformance to the recorded structure. When LOOP_DIR (the resolved `<docs-path>/loops/` from INPUTS) does not exist, create it yourself and seed it: make the folder (`mkdir -p "<LOOP_DIR>"`), then copy the three `references/` prompts into it, prefixing each with its order number - `1.code-quality.md`, `2.naming.md`, `3.comments.md` - and edit to taste. Number them by blast radius - see NOTE ON CONVERGENCE - so later stages do not undo earlier ones. Do this silently as part of the run, then proceed to DISCOVERY; a missing folder is never a reason to pause.
