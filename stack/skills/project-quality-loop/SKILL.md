...
Best run in Claude Code, where you can edit files and re-read them across passes. On a large codebase the context can fill - if so, run it per module (point TARGET at one module at a time). Each stage boundary is a fresh-session resume point - the loops folder plus each file's logged outcome is the handoff; on a long run prefer a new session per stage over one accumulating chat (measured, carried-forward conversation, not tool output, dominates session cost).
