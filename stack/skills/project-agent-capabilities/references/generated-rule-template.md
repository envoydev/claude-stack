...
substituted with the registered name (`playwright` ships as one server per kept browser).
...
- `memory` - the shared memory (required, not a routing option): preferences, corrections, project
  ...
  reading a related project's repo. first call: `ToolSearch select:` plus the `mcp__memory__*` names the session's own listing shows.
- `playwright` - drive a browser for visual checks / large HTML reports - don't text-read them. Screenshots: omit `filename` (auto-names land in the registered output dir, `.playwright/output/`), or prefix an explicit name with `.playwright/output/` - the server resolves explicit filenames against the repo ROOT, so a bare name litters the repo. Readback discipline: verify UI state via `browser_snapshot` / `browser_evaluate` (DOM assertions), or a `target`-scoped screenshot for a localized visual check - a full-page PNG Read is for the FINAL accepted state only, never the iteration loop. first call: `ToolSearch select:mcp__<server>__browser_snapshot,mcp__<server>__browser_evaluate,mcp__<server>__browser_wait_for`, plus the other `mcp__<server>__*` names the session's own listing shows.
- `angular-cli` - the framework CLI's own docs / commands. first call: `ToolSearch select:` plus the `mcp__angular-cli__*` names the session's own listing shows.
- `chrome-devtools` - browser debug, only for that target (native deps: it fails at launch without them). first call: `ToolSearch select:` plus the `mcp__chrome-devtools__*` names the session's own listing shows.
- `appium-mcp` - native-mobile debug, only for that target (native deps: it fails at launch without them). first call: `ToolSearch select:` plus the `mcp__appium-mcp__*` names the session's own listing shows.
- `sentry` - production error monitoring; pull the reported issue / event detail before diagnosing a production error, never from the stack trace alone. first call: `ToolSearch select:` plus the `mcp__sentry__*` names the session's own listing shows.
