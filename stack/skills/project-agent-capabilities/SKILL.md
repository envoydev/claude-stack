...
- A deliberate orchestration skill (a capture, a quality loop, a build flow) starts in a
  ...
  MECHANIZED, not advice: `guard-fresh-session-start.js` blocks the Skill call past ~150k
  context per message and the block is answered with one AskUserQuestion (fresh session,
  recommended, ending the turn with the resume block - or continue here with the cost
  stated). Do not restate the rule as a reminder to 'name the route' - the prose form of
  it lost in 4 of 4 audited sessions, one of which named the route and continued anyway to
  380k per message where the same step cost 134k run fresh.
