...
- Diagnose an error cluster only from the quoted error text, never from counts - hook blocks carry a deterministic `Blocked:` message, a wrong-parameter error names the parameter; grep it before assigning a cause.
