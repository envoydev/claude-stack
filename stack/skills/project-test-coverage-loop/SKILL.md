...
3. **UPDATE DOCS** - re-run the capture: the .NET surface reads 92%, the closed weak points drop off. ASSESSMENT.md listed the `PaymentGateway` `HttpClient` construction as a weakness - the seam fix resolved it, so that entry is pruned too.
