...
- **FIRST capture** (no existing docs, or no `Captured:` stamp) - DELEGATED: dispatch architecture-analyzer per module as below; reasoning and writing stay here. INLINE only when no dispatch exists (Cursor): characterize the modules yourself, serena-first and bounded, and continue at AGGREGATE identically.
