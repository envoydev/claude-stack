...
- `ChangeDetectionStrategy.OnPush` on every component in new code - declared explicitly below v22, the framework default from v22 (which renamed the old default strategy `Default` to `Eager` and deprecated the old name); an eagerly-checked component is treated as a bug. The one sanctioned carve-out is a component a library's own docs require to stay eagerly checked - keep it on `Eager` (`Default` below v22) and cite that requirement in an inline comment.
