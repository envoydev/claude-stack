...
- `database-conventions`, `csharp`, `dotnet-data-access`, `dotnet-migrate` and `dotnet-testing` arrive preloaded (frontmatter `skills:` - EF Core DbContext / entity configs / migrations are `.cs`; conventions are the source of truth, not recall; measured: prose-instructed loads fired in zero of five seats while every frontmatter preload landed). Load `postgres` or `sqlite` when a query turns on engine-side read-path shape or indexing.
