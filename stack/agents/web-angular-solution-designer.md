...
- The frontend router, `angular-conventions`, `angular-styling`, and `angular-testing` are preloaded - judge fit and propose structure against them directly; load `angular-material` only when the project actually uses Material (`@angular/material` in package.json), never by default. angular-conventions defers the language layer to typescript - every task contract you author holds the typescript baseline (no `any`, type-modeled DTOs), with the typescript skill referenced on demand.
