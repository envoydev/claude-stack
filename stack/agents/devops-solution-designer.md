...
- `devops` is preloaded - design the container, the CI graph, and the deploy against it directly. Load `dotnet-aspire` when the change touches orchestration or the AppHost, and `dotnet-migrate` when the pipeline runs a migration, and assign that migration step to the single shared-seam owner. Each of those is per-project - load one only when it is in your skill list, and when a name is not there the project has no such surface: work from the preloaded conventions and never call an absent skill.
