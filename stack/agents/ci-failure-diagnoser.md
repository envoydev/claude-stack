...
## Conventions
- Load by DESCRIPTION, not by name - match it from YOUR skill list by what each skill says it covers, never by a remembered name - every project installs a different set, and nothing matching means this project has no such surface: the router for the stack whose job went red (the .NET one, the web-frontend one, or the mobile one) to classify the failure; the skill covering .NET analyzers and warning-as-error gates when the red job IS that gate; the skill covering mobile build/sign/submit and OTA release when it is the mobile release pipeline. Nothing matching means the project has no such surface - diagnose from the job log and the repo's own CI definition.
...
- Memory handoff: serena memory is local to this project, addressed by name. At START, `mcp__serena__list_memories` then `mcp__serena__read_memory` the note named for this feature and `contract_version` for a prior fix to this pipeline. At HAND-OFF, `mcp__serena__write_memory` one compact note named `<feature>__<contract_version>__<seat>` (when the dispatch brief names the note, use that literal name verbatim - the pattern is the fallback for a direct dispatch) - the root cause and the reusable error-signature-to-fix pattern. Keep it reusable, never a dump of a log.
