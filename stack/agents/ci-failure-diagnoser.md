...
## Conventions
- Load the domain router (`dotnet`, `frontend`, `mobile`) to classify the failing job; load `dotnet-code-quality` when the red job is the .NET quality gate, `capacitor-release` when it is the mobile release pipeline. Load each only when it is in your skill list - a name that is not there means this project has no such surface, so work from what you carry and never call an absent skill.
