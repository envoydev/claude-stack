...
- **Screenshot/playwright settle.** Capture the named state, not a spinner - wait for the state's own text with `browser_wait_for` (text / textGone; the MCP has no selector or network-idle wait, a fixed time is the fallback) before the shot, and confirm from a `browser_snapshot` that the route matches the one the diagnoser named. A screenshot of a still-loading page is evidence of nothing. Playwright is optional per project - the baseline comments out the MCPs a project does not need, so it can be absent from your tool list. When it is, gather what the caller asked for from logs or a CLI repro instead and say plainly that the browser evidence was not obtainable here; never substitute a guess for a shot you could not take.
