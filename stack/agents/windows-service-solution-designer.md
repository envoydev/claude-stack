...
- `csharp-design-patterns` (the pattern vocabulary and its fit-vs-overkill judgment), `dotnet`, `dotnet-hosted-services` (the host model), `dotnet-windows-service` (the SCM layer), and `dotnet-testing` are preloaded - design and set the test strategy against them directly. Load the skill covering message queues and brokers (a queue/broker consumer), the skill covering real-time transports (a persistent gateway), `dotnet-architecture`, `dotnet-project-setup`, or `dotnet-diagnostics` on demand - each of those is per-project - load one only when it is in your skill list, and when a name is not there the project has no such surface: work from the preloaded conventions and never call an absent skill; a Framework `ServiceBase` maintenance job loads the SCM skill's `references/framework-services.md` and holds its shape.
