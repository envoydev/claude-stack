...
- `csharp` arrives preloaded (frontmatter `skills:` - conventions are the source of truth, not recall). Then load the stack skills whose conventions carry the fixes below: `dotnet-hosted-services` for the Generic Host / `BackgroundService` lifecycle, scope-per-work, and stopping-token conventions (the Result/exception two-channel split and the loop try/catch boundary ride the `csharp` error baseline already loaded), `dotnet-testing` for the test approach, and on demand `dotnet-console-apps` (a CLI arg-parsing surface or a bot-SDK integration - `references/bot-sdks.md`), `dotnet-messaging` (a broker consumer), or `dotnet-realtime` (a persistent gateway). The bot integration *shape* is `dotnet-console-apps`; the exact third-party client API is not a house skill - resolve signatures with context7 before writing against them.
