...
- A WPF solution often ships a companion Windows Service / background worker; that process is dotnet-windows-service-implementer's in a fan-out - building such a task inline, load `dotnet-hosted-services` + `dotnet-windows-service` and build it as a worker, not WPF code.
