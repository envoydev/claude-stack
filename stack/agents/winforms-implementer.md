...
- `dotnet-winforms` is preloaded - build against its architecture, binding, disposal, and DPI rules directly, not recall; its runtime reference (`references/net-framework-48.md` on 4.8, `references/modern-net.md` on .NET 8+) carries the concrete mechanics for the workspace's runtime. Load `csharp` before the first `.cs` edit, `dotnet-error-handling` for the failure channels, and `dotnet-testing` for the test approach.
