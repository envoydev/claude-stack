...
- `scripts/install/` - THE INSTALLER, and the default route: `claude-stack.js` is the entry, one
  ...
  in the command bodies. `scripts/os/claude-stack.{sh,ps1}` are the FROZEN twins: kept one release
  behind `CLAUDE_STACK_SEED=shell`, deleted in Phase 7b once one real Windows install through the
  seed is reported, and no longer EDITED - `meta/stack-manifest.json` (generated by
  `scripts/build-manifest.js` from the sh twin, refusing when the ps1 disagrees) is the one source
...
- **The repo root is a plugin source, so seven names are RESERVED there.** Every marketplace entry
  ...
  into a case.
- **Parity / source-of-truth.** Behaviour lands in `scripts/install/` and NOTHING lands in the twins:
  they are frozen until Phase 7b deletes them, and `meta/stack-manifest.json` carries the six lists
  all three read (`npm run lint` enforces the JSON against both twins, plus the HTML and skill
  count). A fix that must reach the shell route before 7b is the exception, not the shape - state it
  in the commit. A shared baseline change is mirrored into cursor-stack in the same sitting. Never
  patch only a generated `.mcp.json` or a consuming project's copy - the installer wipes it.
...
- **Every change is proven on a TEMP PROJECT before it is committed - MANDATORY, no exceptions.** Unit
  ...
  - Edge cases are REQUIRED, not optional: a fresh install; an update over an older install; a re-run
    ...
    file, garbage JSON, unset env); BOTH ROUTES - the Node seed and, behind `CLAUDE_STACK_SEED=shell`,
    the twin the change could still reach (`pwsh` for the `.ps1` when installed, else say it was not
    run); and every boundary the change introduces (at, one under, one over). A change to the seed is
    proven by SEED PARITY as well: the same selection installed both ways into two identically-named
    projects, and the two trees diffed - only the stamp's own timestamp may differ.
...
- **`.mcp.json` is the COPY ROUTE only** (`CLAUDE_STACK_MCPS_VIA_PLUGIN=false`); on the default
  ...
  server the project added by hand is never touched. `scripts/mcp-verify.test.js` pins it on both twins, `scripts/install-mcp.test.js` on the seed.
...
- **Everything installs from ONE source snapshot** per run (`install/source.js`, `stack_src` / `Get-StackSrc`), resolved in
...
- **The plugin cache IS the snapshot, so the common run downloads NOTHING** (`_stack_plugin_cache` /
  ...
  `STACK_SOURCE_CACHE` are all RETIRED. A shape change is a FIVE-site edit now
