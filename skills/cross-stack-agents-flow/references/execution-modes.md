...
These values illustrate the per-mode floor and rationale; they are not a per-dispatch dial. What the orchestrator can and cannot vary per dispatch - model yes (including the guarded-Haiku implementer down-dispatch), effort no - is the asymmetry `references/model-routing.md` owns. So the primary cost lever is the MODE / seat-count, not a re-dialed seat: `single_chat` and `implementer_only` skip the designer and the verifier entirely, which saves far more than running any dispatched seat a shade cheaper. A heavier need escalates to a higher mode or a heavier seat (task-analyzer -> architecture-analyzer, a domain verifier -> integration-reviewer), per the same policy. Capability wiring - context7 before a library API, a memory note read instead of a re-derivation - is the other lever the mode carries; see `references/capability-reuse.md`.
