...
This skill ships a starter set under its own `references/` folder - four stage prompts: code quality, naming, comments, and tests (the last is gate-based; the rest are audits). The code-quality stage reads `docs/architecture/ARCHITECTURE.md` and audits TARGET for both quality and conformance to the recorded structure. When LOOP_DIR (default `docs/loops/`) does not exist, create it yourself and seed it: make the folder (`mkdir -p docs/loops`), then copy the four `references/` prompts into it, prefixing each with its order number - `1.code-quality.md`, `2.naming.md`, `3.comments.md`, `4.tests.md` - and edit to taste. Number them by blast radius - see NOTE ON CONVERGENCE - so later stages do not undo earlier ones. Do this silently as part of the run, then proceed to DISCOVERY; a missing folder is never a reason to pause.
