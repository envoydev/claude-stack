...
description: "Autonomous review-and-fix loop pipeline driven from a folder of numbered prompt files. Runs each prompt in numeric order, looping it on a target until its bar (default: zero findings at every severity) is met, then advances - making and logging judgment calls itself, never pausing for input. Use when running a multi-stage review/polish pipeline (e.g. a loops/ folder of code-quality/naming/comments/tests prompts) over a codebase or module; triggers on 'run the project quality loop' or 'run the loops pipeline'. Code-quality only - architecture restructuring is the separate architecture-quality-loop skill. A folder-driven, multi-pass, fix-and-re-run pipeline, distinct from the single-shot /code-review and /security-review (one diff sweep vs an iterated stage-by-stage polish to a bar; one forward pass, re-run for a fixpoint). Do NOT fire for a single ad-hoc review pass, without a folder of prompt files, or when the user wants findings reported without auto-fixing - in a stack-installed project it drives the domain verifiers, implementers and resolvers from the main session."
...
This skill ships a starter set under its own `references/` folder - four stage prompts: code quality, naming, comments, and tests (the last is gate-based; the rest are audits). The code-quality stage reads `docs/architecture/ARCHITECTURE.md` and audits TARGET for both quality and conformance to the recorded structure; architecture-level restructuring is out of scope here - that is the `architecture-quality-loop` skill. Copy them into your LOOP_DIR (default `loops/`), prefix each with its order number, and edit to taste. Number them by blast radius - see NOTE ON CONVERGENCE - so later stages do not undo earlier ones.
