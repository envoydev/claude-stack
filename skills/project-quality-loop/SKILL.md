...
One deployment caveat: in a project that carries the convention-gate hook (require-convention-skill), an unattended edit to a gated file - a .cs, .ts, hand-written .sql, or an Angular component/service file - is hard-blocked until that file type's convention skill is loaded this session. Before the loop starts editing in such a project, make sure the relevant skill (`csharp`, `typescript`, or `angular-conventions`) is already active; otherwise every FIX exit-2's and the inner loop thrashes without ever converging.
...
## DOMAIN CONVENTIONS - .NET / Angular targets
