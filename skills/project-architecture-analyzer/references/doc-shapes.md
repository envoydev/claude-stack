...
3. **Project structure** - a module-inventory table, one row per module/project: module | path + entry point | responsibility | depends on (why). One row carries everything about a module in one place - the grouped shape agents resolve dependencies from most reliably - and doubles as the concern-to-place map.
...
Format discipline - every seat reads this map at orientation, so its weight is paid on every dispatch: keep the core file within ~150 lines and spill the rest to `references/`. Tables and lists for anything enumerable; prose only for reasoning structure cannot carry (why a boundary exists, an accepted tradeoff). Never ASCII-art box diagrams - the same graph costs about double the tokens and models misread 2D text layout.
