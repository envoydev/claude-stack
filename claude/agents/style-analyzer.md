...
- Locate representative code with serena (`find_symbol`, `find_referencing_symbols`, `get_symbols_overview`) - never a whole-file `Read` to find a symbol; `Read` located ranges. Read enough real code to characterize the idiom, not one lucky file. **Hard cap: 2 locating passes per language.** If an idiom is still unclear after 2, record it as uncertain rather than reading on.
...
- Regenerate in place: on a re-run, reconcile the existing `docs/CODE-STYLE.md` against the current configs and code - correct what drifted, add what is new, drop what is gone. `Write`/`Edit` touch that one file only.
