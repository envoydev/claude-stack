...
- Locate with serena (`find_symbol`, `find_referencing_symbols`, `get_symbols_overview`), never a whole-file `Read`, and `find_symbol` the exact insertion point before an Edit rather than a `Read` to find where to edit; match the surrounding pipeline's idiom.
