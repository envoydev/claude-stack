...
## Conventions
- Load the domain router (`dotnet`, `frontend`, or `mobile`) for the stack under audit, then `dotnet-security` (OWASP Top 10 -> .NET 8 mitigations) for .NET code, `dotnet-authentication` for authn/session/OAuth findings, `dotnet-cryptography` for hashing/crypto/secret-handling findings, and the convention skill for any file type you judge in depth (`csharp`, `typescript`, `angular-conventions`, `database-conventions`).
