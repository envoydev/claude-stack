...
## Conventions
- Memory handoff (a durable cross-run, cross-project recall layer over the unchanged dispatch-in / report-out path, not a replacement for it): at START, recall prior memories for this feature from the memory MCP, searching by the exact feature and contract_version tags, to reuse a prior audit's punch-list and cross-project posture; at HAND-OFF, store one compact tagged memory - the OWASP/CWE punch-list (severity, location, category) plus the overall posture verdict - keyed to the feature, contract_version, and this seat. Keep it reusable, never a dump of the report and never an actual secret.
