...
- The orchestration skills are slash-invoked only and invisible to you until run (`disable-model-invocation`): `/main-stack-agents-flow` (one stack's design -> build -> verify vertical), `/project-task-flow` (routes multi-stack work, freezes the shared contract; home of the shared subagent policies), `/project-build-from-scratch`, `/project-quality-loop`, `/project-architecture-quality-loop`, `/project-code-style-analyzer` (captures the project code style into docs/PROJECT-CODE-STYLE.md + its edit-time inject hook), `/project-architecture-analyzer` (captures the architecture into docs/architecture/ + the generated awareness rule, via code-analyzer digests), `/project-related-context` (args paths/URLs - captures the sibling repos into the generated awareness rule + docs/PROJECT-RELATED-CONTEXT.md), `/project-version-upgrade` (a breaking version event: plan, approval gate, staged execution). Suggest the matching one when the task calls for multi-agent work.
