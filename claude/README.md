...
| **Rules** | 9 | the always-on `house-baseline` (no `paths:` - the cross-project working conventions, loaded every session and subagent like CLAUDE.md but refreshed on `update`) + markdown authoring + per-language repair-loop routing + the 5 convention rules (angular/web/ionic, asp.net, wpf, sql, devops - each glob-attaches a file type to its house-style skill, the soft replacement for the retired convention-gate hook), the other 8 path-scoped → `.claude/rules/` |
