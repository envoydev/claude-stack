...
| **Rules** | 16 | six always-on `baseline-*` rules (no `paths:` - the cross-project working conventions grouped by exclusion affinity: interaction (communication + proposal review + planning), quality-gates (code quality + definition of done), security, git + pre-commit, navigation, agents-skills; the related-projects awareness schema moved into CLAUDE.template.md's '## Related projects' section, populated via the /project-related-context skill; loaded every session and subagent like CLAUDE.md but refreshed on `update`, individually excludable; MCP routing lives in each project's CLAUDE.md table) + ten path-scoped: markdown authoring, the two repair-loop routers, and seven single-job convention rules (typescript, angular, angular-styling, csharp, wpf, sql, devops - each glob-attaches ONE file family to its house-style skill, so a stack a project lacks is simply not installed) → `.claude/rules/` |
