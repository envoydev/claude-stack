...
| **MCP servers** | 6 | `angular-cli`, `serena`, `playwright`, `context7`, plus `chrome-devtools` + `appium-mcp` (heavy - active; comment out where not needed); `memory` is now opt-in (commented - cross-project recall only) → `<repo>/.mcp.json` |
...
| **uvx** (uv) | `serena` (+ `memory` if enabled) MCPs | for those MCPs | `curl -LsSf https://astral.sh/uv/install.sh \| sh` | `powershell -ExecutionPolicy Bypass -c "irm https://astral.sh/uv/install.ps1 \| iex"` |
