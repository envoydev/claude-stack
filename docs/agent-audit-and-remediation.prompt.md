...
3. Apply the smallest edit that removes the deduction. Examples:
   ...
   - Named cite of a skill or MCP server the install can lack: replace the name with what it covers plus the nothing-matches path, and add a described skill to `suggests:`; never a guard phrase beside the name.
