...
- A stalled hook is not a gate. A `command` hook with no `timeout` gets the 600-second default, and a timed-out `command` / `http` / `mcp_tool` hook does NOT block the tool call. Every wired entry carries a short timeout, and the per-call latency is a measured number.
