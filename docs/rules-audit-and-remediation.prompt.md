...
- Installs are selective: a rule deploys as a single file, and the artifact it points at may be excluded from a given install. A load-bearing pointer is safe only when the same install unit guarantees the target (a convention rule and its house-style skill install as one stack unit); a pointer beyond that guarantee must be conditional-by-design - phrased to degrade gracefully when the target is absent - and operative content is never loaded from another artifact's file, because an install-time-optional artifact can never be a load-bearing dependency.
