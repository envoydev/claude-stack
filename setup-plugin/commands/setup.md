...
Then apply the step-1 environment choices where they differ from what the installer left: a merge on the scope's settings.json touching ONLY the chosen keys - `env.CLAUDE_AUTOCOMPACT_PCT_OVERRIDE`, `env.CLAUDE_STACK_DOCS_PATH`, and `env.CLAUDE_STACK_INSTRUMENT`
