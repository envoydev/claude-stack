...
- `tsconfig.json` / `jsconfig.json` -> `typescript` (the language-level seed - conventions rules + LSP plugin + typescript-testing). A TS framework stack claims its own surface: when `web-angular` / `ionic-angular` / `browser-extension` detects, do NOT also report `typescript` for the same app - the framework seeds already carry the rules + LSP, and testing is `angular-testing`'s there (browser-extension seeds typescript-testing itself); report both only when the repo holds a genuine non-framework TS surface too (Node tooling, a published library, scripts with their own tests).
...
Hooks are leaf picks - nothing requires them, they require nothing, so every row is free. Recommended = the four always-on guards; `instrument-tool-usage` is the opt-in extra (installed unwired - it only runs when a project wires it deliberately). The installer wires the selected hooks into `.claude/settings.json` on install.
