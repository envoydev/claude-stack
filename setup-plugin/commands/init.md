...
# Initialize the Claude stack - fresh install
...
model's row in `.claude/hooks/model-windows.json`, else `CLAUDE_STACK_DEFAULT_CONTEXT_WINDOW` (a FIRST setup has neither yet: take
...
Hooks are leaf picks - nothing requires them, they require nothing, so every row is free. Recommended = all sixteen: the eleven always-on guards, the session monitor (`monitor-session` - never denies; seeded to log its notes, not inject them), the two session engines (`docs-session` - the docs start block and finish ask - and `memory-session` - the shared-memory slice at session start), the turn-end build check (`check-turn-build` - wired, but inert until `CLAUDE_STACK_TURN_CHECK` flips to `1`, seeded `0`), plus the env-gated `instrument-tool-usage` (wired like the guards, but inert until `CLAUDE_STACK_INSTRUMENT` flips to `1` - so keeping it costs nothing idle, and dropping it leaves the install unable to record a measured run without a manual re-wire). The whole set now ships together in the `claude-stack-hooks` plugin, so nothing is copied and nothing is wired per project: this answer is written as `CLAUDE_STACK_HOOKS_OFF` in the project's `.claude/settings.json` env - the rows you DROP are the ones named there, and **None** names all sixteen (the emitted selection carries `hook none` for it; `--hooks-answered` at step 10 is what writes it) - and it can be changed later by editing that value, no reinstall.
...
- `hooks switched off:` the names in `written.hooksOff` (`CLAUDE_STACK_HOOKS_OFF`), or `none` - on
...
the install writes. Under `CLAUDE_STACK_SEED=shell` the frozen twin writes no seat deny: print
