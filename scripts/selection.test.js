...
test('sh: hook lines filter hooks; a selection without them keeps the install-all legacy behavior', () => {
    ...
    assert.deepStrictEqual(planLine(legacy, 'hooks'),
        ['guard-protected-force-push', 'guard-catastrophic-rm', 'guard-read-whole-file', 'guard-unapproved-dispatch', 'guard-ungated-commit', 'guard-stop-contract', 'guard-answer-length', 'instrument-tool-usage'],
