...
test('corpus-replay: one Bash payload fans out to every Bash-wired guard', () => {
  ...
  for (const h of ['protected-force-push', 'catastrophic-rm', 'read-whole-file', 'secret-value', 'ungated-commit', 'cross-project-write', 'config-protection']) {
...
test('corpus-replay: identical payloads are replayed once', () => {
  ...
  assert.match(err, /unique replay jobs: 7\b/, 'but dedupe leaves one job per Bash route');
});
